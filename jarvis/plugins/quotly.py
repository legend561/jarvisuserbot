"""QuotLy: Avaible commands: .qjjbot
"""
...
@jarvis.on(admin_cmd(pattern=r"qjjbot(?: |$)(.*)", outgoing=True))
@jarvis.on(sudo_cmd(pattern=r"qjjbot(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with jjbot.conversation(chat) as conv:
        ...
        if response.text.startswith("Hi!"):
            ...
            await jjbot.forward_messages(event.chat_id, response.message)
