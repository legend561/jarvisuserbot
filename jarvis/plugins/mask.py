...
async def _(event):
    ...
    async with jjbot.conversation(chat) as conv:
        try:
            ...
            await jjbot.send_message(chat, reply_message)
            response = await response
        except YouBlockedUserError:
            await event.edit("```Please unblock @sangmatainfo_jjbot and try again```")
            return
        if response.text.startswith("Forward"):
            ...
            await jjbot.send_file(event.chat_id, response.message.media)
