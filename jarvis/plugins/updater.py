...
GIT_REPO_NAME = "JarvisUserjjbot"
UPSTREAM_REPO_URL = "https://github.com/Jarvis-Works/jarvisuserjjbot"
...
async def upstream(ups):
    "For .update command, check if the jjbot is up to date, update if specified"
    ...
    try:
        ...
        if conf != "now":
            ...
            \nBut we can fix that by force updating the userjjbot using** `.update now`."
            )
            return
        repo = Repo.init()
    ...
    if force_updateme:
        await ups.edit("`Force-Syncing to latest stable userjjbot code, please wait...`")
    else:
        await ups.edit("`Updating userjjbot, please wait....`")
    ...
    if Var.HEROKU_API_KEY is not None:
        ...
        if not Var.HEROKU_APP_NAME:
            await ups.reply(
                "`Please set up the HEROKU_APP_NAME variable to be able to update userjjbot.`"
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await ups.reply(
                f"{txt}\n`Invalid Heroku credentials for updating userjjbot dyno.`"
            )
            repo.__del__()
            return
        await ups.edit(
            "`Userjjbot dyno build in progress, please wait for it to complete.`"
        )
        ups_rem.fetch(ac_br)
