...
async def forw(event):
    ...
    for channel in channels:
        try:
            await jbot.forward_messages(int(channel.chat_id), previous_message)
            sent_count += 1
            await event.edit(
                f"Sent : {sent_count}\nError : {error_count}\nTotal : {len(channels)}"
            )
            ...
            try:
                ...
                await jbot.send_message(logs_id, "Error! " + str(error))
    ...
    if error_count > 0:
        try:
            await jbot.send_message(logs_id, f"{error_count} Errors")
        except:
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if not previous_message.web_preview and previous_message.photo:
            file = await jbot.download_file(previous_message.media)
            uploaded_doc = await jbot.upload_file(file, file_name="img.png")
