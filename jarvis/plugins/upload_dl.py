""" Userjbot module which contains everything related to \
...
async def download(target_file):
    """ For .dl command, download files to the userjbot's server. """
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the userjbot's server """
    await eor(u_event, "Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("userjbot.session", "config.env"):
