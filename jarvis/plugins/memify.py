...
async def mim(event):
    if not event.reply_to_msg_id:
        await eor(
            event,
            "`Syntax: reply to an image with .mmf` 'text on top' ; 'text on jjbottom' ",
        )
        return
    ...
    if reply_message.photo:
        dls_loc = await jjbot.download_media(
            reply_message,
            "meme.png",
        )
    elif (
        ...
        video = await jjbot.download_media(
            reply_message,
            "meme.mp4",
        )
