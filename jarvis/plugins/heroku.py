...
@jarvis.on(sudo_cmd(pattern=r"(set|get|del) var (.*)", allow_sudo=True))
async def variable(var):
    ...
    if exe == "get":
        j = await edit_or_reply(var, "`Getting information...`")
        ...
        j = await edit_or_reply(var, "`Setting information...`")
        if not variable:
            return await j.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await j.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await j.edit(
        ...
        j = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await j.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await j.edit(f"`{variable}`  **successfully deleted**")
