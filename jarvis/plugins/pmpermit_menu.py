...
from jarvis import ALIVE_NAME, jjbot
...
async def _(event):
    ...
    if not pmpermit_sql.is_approved(chat_id):
        ...
        FIVE = "`Okay. please have the basic manners as to not jjbother my master too much. If he wishes to help you, he will respond to you soon.`\n**Kindly Do not ask repeatdly else you will be blocked and reported.**"
        ...
        async with jjbot.conversation(chat) as conv:
            if pmpermit_sql.is_approved(chat_id):
                return
            await jjbot.send_message(chat, PM)
            chat_id = event.sender_id
            chat = await event.get_chat()
            ...
            if y == "1":
                if pmpermit_sql.is_approved(chat_id):
                    return
                await jjbot.send_message(chat, ONE)
                response = await conv.get_response(chat)
                await event.delete()
                if not response.text == "/start":
                    ...
                    if pmpermit_sql.is_approved(chat_id):
                        return
                    await jjbot.send_message(chat, LWARN)
                    response = await conv.get_response(chat)
                    await event.delete()
                    await response.delete()
                    response = await conv.get_response(chat)
                    if not response.text == "/start":
                        if pmpermit_sql.is_approved(chat_id):
                            return
                        await jjbot.send_message(chat, TWO)
                ...
                if pmpermit_sql.is_approved(chat_id):
                    return
                await jjbot.send_message(chat, FOUR)
                response = await conv.get_response(chat)
                await event.delete()
                await response.delete()
                if not response.text == "/start":
                ...
                if pmpermit_sql.is_approved(chat_id):
                    return
                await jjbot.send_message(chat, FIVE)
                response = await conv.get_response(chat)
                if not response.text == "/start":
                    if pmpermit_sql.is_approved(chat_id):
                        return
