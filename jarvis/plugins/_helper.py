...
async def cmd_list(event):
    ...
    if tgjjbotusername is None or input_str == "text":
        string = ""
        for i in CMD_LIST:
            string += "💗" + i + "\n"
        ...
        help_string = f"""Userjjbot Helper.. Provided by 💗{DEFAULTUSER} \n
Jarvis Helper to reveal all the commands\n__Do .help plugin_name for commands, in case popup doesn't appear.__"""
        results = await jjbot.inline_query(  # pylint:disable=E0602
            tgjjbotusername, help_string
        )
...
async def _(event):
    if event.fwd_from:
        return
    result = await jjbot(functions.help.GetNearestDcRequest())  # pylint:disable=E0602
...
async def _(event):
    if event.fwd_from:
        return
    result = await jjbot(functions.help.GetConfigRequest())  # pylint:disable=E0602
