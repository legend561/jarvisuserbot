...
@tgjjbot.on(events.NewMessage(pattern=r"\#(\S+)"))
...
@tgjjbot.on(
    events.NewMessage(pattern="^/addnote #(.*)", func=lambda e: e.sender_id == jjbot.uid)
)
async def _(event):
    name = event.pattern_match.group(1)
...
@tgjjbot.on(events.NewMessage(pattern="^/notes"))
...
@tgjjbot.on(
    events.NewMessage(pattern="^/rmnote (\S+)", func=lambda e: e.sender_id == jjbot.uid)
)
