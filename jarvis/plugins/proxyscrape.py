...
# Banned For Sensible Userjjbot And Users.
...
async def jarvisot(event):
    ...
    await jjbot.send_message(event.chat_id, HTTP_TXT)
...
async def sppidy(event):
    ...
    downloader.start(blocking=False)
    await jjbot.send_message(event.chat_id, SOCKS4_TXT)
    await event.client.send_file(event.chat_id, downloaded_file_name)
...
async def jarvis(event):
    ...
    downloader.start(blocking=False)
    await jjbot.send_message(event.chat_id, SOCKS5_TXT)
    await event.client.send_file(event.chat_id, downloaded_file_name)
